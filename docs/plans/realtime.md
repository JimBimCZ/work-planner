...
- [x] **A reseed does not erase an optimistic change made during the gap** — checked, and **the box's premise was wrong in a way that hid a crash.** There is no offline write queue and the plan never asked for one, so a card added while offline cannot "settle once you come back": the server action is a `fetch`, and offline it rejects at once with `TypeError: Failed to fetch`. Probing that is what found the real defect, which was far worse than a lost card — the rejection escaped `startTransition` into the nearest error boundary and **replaced the entire board with "This page couldn't load"**, taking every column, every card on screen and the status strip that would have explained it. Every one of the eight server-action call sites in `components/board/` had the same shape and none had a `try`. Fixed by `lib/attempt.ts`; proved by `a write that cannot reach the server leaves the board standing` and `a rename that cannot reach the server rolls back and says so`. The reseed cannot erase an optimistic change made offline because no such change survives the failed write — it rolls back and says so, which is the correct behaviour and is now asserted.
- [ ] A reseed never fires mid-drag. **Still code, not evidence, and left open deliberately.** `components/board/board-canvas.tsx`'s catch-up effect returns early on `draggingId`, which is in its dependency array, so the reseed lands after the drag rather than during it. Committing a test for it needs a way to hold a server action open while a reconnection lands — the temporary-edit technique board-canvas Section E used for its rejected-move check, which verifies once by hand but leaves nothing behind. The narrower risk the guard exists for is now partly covered from the other side: a write that cannot reach the server no longer takes the board down, so the worst case here is a stale board rather than a destroyed one.
...
- [x] A client that is disconnected and reconnected converges without a reload — `a client that missed events catches up on reconnect`, which opens the gap with `setOffline(true)` so `pusher-js` sees a real disconnection, moves a card in the other browser, and asserts B converges without ever reloading.
- [x] **...and does so without erasing an optimistic change made during the gap.** Closed by the Section 4 gate above, which found the premise wrong and a crash behind it. Original note kept for the record: **not evidenced at the time.** This half was ticked in error, citing a test named `a catch-up does not erase a change made while disconnected` that does not exist and never did — the suite's only reconnect test makes no local change on the disconnected client. What is true is narrower and worth writing down, because it is the whole of the protection: `board.reseed` **replaces the entire state** (`lib/board-state.test.ts`, "replaces the whole state") and preserves nothing, so nothing in the reducer defends an optimistic card. The defence is one line in `components/board/board-canvas.tsx` — `if (draggingId || isPending) return;` — plus the effect re-running when those flip, and it has no test at any level. The by-hand check at the Section 4 gate is still the only thing that would close this, and it is still open.
