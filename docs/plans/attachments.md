...
All five must be `0`, each read from its own log. `pnpm build` is not optional here: it is the only check that catches a server-only module reaching the client bundle.
