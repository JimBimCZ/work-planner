...
- [ ] Screenshots of the modal and the canonical page, both themes, in the PR body. **Not satisfied, and left unticked deliberately.** All four exist and were regenerated at HEAD — the modal and the canonical page, light and dark — but they were handed to the reviewer directly instead of embedded. PR #54 says where they are. **The stated reason is wrong.** `gh` has no image-upload endpoint, but that was never the only route: an image committed under `docs/screenshots/` embeds in a PR body with a `https://github.com/JimBimCZ/work-planner/blob/<branch>/<path>?raw=true` URL. Realtime Section 5 (PR #63) did exactly that, and Section 7 (PR #65) repeated it. This box is open because the images live in gitignored `.superpowers/`, not because GitHub refused them.
...
- [ ] Screenshots of a board with overdue, soon and plain cards, both themes, in the PR body. Section 3 recorded the same blocker (see 8610647). Screenshots exist at `.superpowers/sdd/card-modal/shots/`. **The stated reason is wrong.** `gh` has no image-upload endpoint, but that was never the only route: an image committed under `docs/screenshots/` embeds in a PR body with a `https://github.com/JimBimCZ/work-planner/blob/<branch>/<path>?raw=true` URL. Realtime Section 5 (PR #63) did exactly that, and Section 7 (PR #65) repeated it. This box is open because the images live in gitignored `.superpowers/`, not because GitHub refused them.
...
- [ ] Screenshots of a thread, both themes, in the PR body. **Not satisfied, and left unticked deliberately** — the same blocker Sections 3 and 4 recorded. The images exist at `.superpowers/sdd/card-modal/shots5/` and PR #57 says so. **The stated reason is wrong.** `gh` has no image-upload endpoint, but that was never the only route: an image committed under `docs/screenshots/` embeds in a PR body with a `https://github.com/JimBimCZ/work-planner/blob/<branch>/<path>?raw=true` URL. Realtime Section 5 (PR #63) did exactly that, and Section 7 (PR #65) repeated it. This box is open because the images live in gitignored `.superpowers/`, not because GitHub refused them.
