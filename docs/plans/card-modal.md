...
- [x] Soft navigation renders the intercepted probe and leaves the canvas mounted. Observed: A real `next/link` (temporarily added to `boards/[boardId]/page.tsx` for this test only, reverted after) clicked from `/boards/<id>` to `/boards/<id>/cards/<cardId>` produced **zero** `document`-resourceType network requests (checked via Playwright's `page.on('request')`), i.e. no full-page load. After the click: `[data-probe="intercepted"]` count = 1, `[data-probe="canonical"]` count = 0, `[data-column-id]` count = 5 (all five default columns of the seeded board still in the DOM — the canvas stayed mounted), and `window.location` updated to the card URL. Verified via a Node script driving `@playwright/test`'s `chromium.launch()` against `pnpm dev` on port 3000, DOM read with `.count()`/`.innerText()`, not a screenshot.
- [x] A hard load renders the canonical probe and no modal. Observed: `context.newPage()` (fresh browsing context, same session cookie) navigating directly to `/boards/<id>/cards/<cardId>` returned HTTP 200; `[data-probe="canonical"]` count = 1, `[data-probe="intercepted"]` count = 0, `[data-column-id]` count = 0 (the board's own `page.tsx`, which renders `BoardCanvas`, is not on this route — the URL matches the sibling `cards/[cardId]/page.tsx`, not `boards/[boardId]/page.tsx`, so this absence is structural, not a defect). Rendered body text was exactly `CANONICAL <cardId>` under the board's TopBar.
- [x] `@card/default.tsx` removed: what a hard load of the plain board does. Observed: with `default.tsx` deleted, `curl` (with the seeded session cookie) against the plain `/boards/<id>` returned **`STATUS=404`**. The dev server's own request log confirmed it server-side: `GET /boards/<id> 404 in 441ms`. Response body was Next's real error shell (`<html id="__next_error__">`) carrying `data-next-error-digest="NEXT_HTTP_ERROR_FALLBACK;404"` and the text "page could not be found" — a genuine 404, not a silently blank slot. Restoring `default.tsx` and re-requesting the same URL (no server restart, Turbopack picked up the file) returned `STATUS=200` again. This matches what the Next docs claim, confirmed against Next.js 16.3.3 (from `pnpm dev`'s own startup banner) rather than assumed from the docs alone.
- [x] `git status --short` is clean of probe files; `app/(app)/(board)/boards/[boardId]/` holds only `layout.tsx` and `page.tsx`. Observed: after deleting `@card/` and `cards/` and running `git checkout --` on both `layout.tsx` and the temporarily-modified `page.tsx` (which carried the throwaway probe `next/link` used for the soft-navigation check — outside the brief's four listed files, added and fully reverted for this test), `git status --short` produced no output and `git status` reported "nothing to commit, working tree clean". `ls "app/(app)/(board)/boards/[boardId]/"` shows exactly `layout.tsx` and `page.tsx`.
- [x] Deleting the probe route files left `pnpm typecheck` transiently failing (`EXIT=2`): `.next/dev/types/validator.ts`, Turbopack's route-typegen cache, still imported the removed `@card/(.)cards/[cardId]/page.tsx` and `cards/[cardId]/page.tsx`. Not a real regression — the gitignored cache was stale from `pnpm dev` having run while those files existed. `rm -rf .next` cleared it; `pnpm typecheck` then passed with `EXIT=0`. If a future session hits the same symptom after deleting route files mid-dev-session, this is the cause and the fix.
- [x] Open the PR with the findings as the body. Say plainly that no behaviour changed. Stop. Start Section 2 in a fresh session. — PR #52.
...
- [x] **Production is migrated by hand before this merges**, not after. Vercel deploys from `main` and CI cannot gate it:
      `MIGRATE_URL="$(npx --yes neonctl@4 connection-string main --project-id withered-glade-54206401)" pnpm db:migrate`
      Observed: **this ran after the merge, not before — the gate was missed.** Checked on 2026-08-31 with `main` already merged and deployed: production carried 3 migrations, no `comments` table, and `cards.created_by_id` still `NOT NULL`. Nothing was broken by the window, because Section 2 shipped schema only and no deployed code reads `comments` yet. The migration was then run against `main` of `withered-glade-54206401` (EXIT=0) and confirmed against the catalog rather than the success line: tables now `account, board_members, boards, cards, columns, comments, session, user`; 4 migrations applied; `comments.author_id` and `cards.created_by_id` both `is_nullable = YES`; `pg_constraint` `confdeltype` **n** for `comments_author_id_user_id_fk` and `cards_created_by_id_user_id_fk`, **c** for `comments_card_id_cards_id_fk`, all `convalidated = true`; `comments_card_id_created_at_idx` present.
- [x] CI is green on the PR — that is what proves the migration applies to an empty database. Observed: PR #53's `verify` check concluded SUCCESS, and the PR merged at 2026-08-31T13:39:55Z.
...
- [x] Screenshots of the modal and the canonical page, both themes, in the PR body — `docs/screenshots/card-modal-section-3/{modal,canonical}-{light,dark}.png`, embedded in PR #54. **Recaptured rather than reused:** the originals in gitignored `.superpowers/` predate three realtime sub-projects touching this surface, so embedding them would have shown UI that no longer exists. Recaptured a second time on top of #69, which fixed a collision these very shots exposed — the modal's close button was sitting on the title input's top-right corner, a measured 16x16px overlap that also took the click from that corner of the field.
- [x] Open the PR. Stop. Start Section 4 in a fresh session. — PR #54.
...
- [x] Clearing the date empties it rather than erroring. **This was false when first claimed** — the control was fully parent-controlled and dropped the empty `change`, so React 19's controlled-date restore snapped the old value back and blur never saw `''`. Fixed in `79c1089`. Observed by the controller in a real browser afterwards: clearing leaves the field empty with no write, blur fires exactly one POST, and the field is still empty after a reload. The new e2e was watched failing against the pre-fix code before the fix landed. **A second bug survived that fix**: the `79c1089` draft committed on blur only, and Escape and browser-back both close the modal without blurring the field, so a clear made by Delete-then-Escape (or Delete-then-back) was silently lost — the field showed empty, the modal closed, nothing was written. Reproduced in real Chromium at `d4858d8` on all five gestures (Delete/Backspace + click-away/backdrop all wrote; Escape and browser-back both wrote nothing). Fixed by lifting the draft into `CardBody` alongside `title`/`description`, adding it to `useCardEscapeGuard`'s dirty check so the first Escape reverts the draft instead of closing the modal, matching the title and description fields' existing Escape-reverts behaviour. Browser-back is unchanged and out of scope, per the spec's accepted browser-back trade for title and description. New e2e `clearing a due date and pressing Escape keeps the card open and reverts the field` was watched failing against the pre-fix code (`dialog` never became visible) before this fix landed.
...
- [x] Screenshots of a board with overdue, soon and plain cards, both themes, in the PR body — `docs/screenshots/card-modal-section-4/due-dates-{light,dark}.png`, embedded in PR #55. All four states in one shot: overdue in rust with its mono `3d over` label, soon in amber, a distant date muted, and a card with no date at all. Captured at HEAD.
...
- [x] Screenshots of a thread, both themes, in the PR body — `docs/screenshots/card-modal-section-5/thread-{light,dark}.png`, embedded in PR #57. Recaptured at HEAD for the same reason Section 3's were.
