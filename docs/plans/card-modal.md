...
- [ ] Screenshots of the modal and the canonical page, both themes, in the PR body. **Not satisfied, and left unticked deliberately.** All four exist and were regenerated at HEAD — the modal and the canonical page, light and dark — but the `gh` CLI has no endpoint for uploading an image into a PR body, so they were handed to the reviewer directly instead of embedded. PR #54 says where they are. Whoever attaches them can tick this.
