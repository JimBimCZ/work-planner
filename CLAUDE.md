...
- Payloads carry the changed entity, not a full board refetch, and stay under `PAYLOAD_CEILING` (8,192 bytes, headroom under Pusher's documented 10KB). The two fields that cannot fit are handled by saying so and letting the reader ask:
  - `card.updated` carries `descriptionChanged: boolean` rather than the description; an open card calls `readCardDescription`. It **also** carries `descriptionPreview` — not the same thing, and the distinction is what keeps the rule intact: the description is unbounded at 10,000 characters, while the preview is cut to `DESCRIPTION_PREVIEW_MAX` (140) *before* it reaches the payload, at roughly 1.7% of the ceiling. The key is **optional, and only `setCardDescription` sends it**: `renameCard` and `setCardDueDate` publish the same event knowing nothing about the description, and a key they sent would blank the card face on every other client. The reducer reads an absent key as "leave it" and an explicit null as "clear it", the way it already does for `dueDate`.
...
- The card face carries two clamped lines of the description under the title, in the body family rather than the mono one — it is prose, not data. `line-clamp-2` is load-bearing rather than cosmetic: a card whose height varied with its description would reflow its column under a drag in progress, the same reason `LabelLine` truncates rather than wraps. The text is cut to `DESCRIPTION_PREVIEW_MAX` in two places and neither is redundant: `left()` in `getBoardWithColumns` keeps 10,000 characters per card out of the board's RSC payload, and `previewOf` keeps them out of the Pusher payload. Postgres `left()` counts characters while JS `slice` counts UTF-16 units, so an emoji-dense description can differ by a character between the optimistic preview and the next reload — it is a truncated preview, and that is not worth a fix.
